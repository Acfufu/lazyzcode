...
- **0.0.6 第一批已落地（2026-09-15，goal v006-quality-batch1）**：0.0.6 清单 §⑦ 重整（跨平台以侦察项入列，拍板=侦察不承诺支持，待产出 research-crossplatform 底稿+启动器设计稿）+ 双证据红绿纪律落地协议/技能文本层（SKILL Evidence 红绿半+一行豁免、comparator 增查两半在场、qa-executor 契约同步、guide 双语 bullet，CLI 零改动）+ 对抗类清单（docs/research-adversarial-checklist.md：九类三态判决 8 已有防护+1 当轮小修、记账新债 3 条带升格条件、OMZ 分类学事实引用+SUL-1.0 边界头注）；AGENTS 三条 09-14 行合一腾位+§8 增「双证据」词条；计划评审三轮收敛 PASS。
