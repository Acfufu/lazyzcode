...
- **0.0.6 第一批已落地 + 跨平台拍板落地（2026-09-15，goal v006-quality-batch1 / crossplatform-recon，两目标行合一腾位）**：第一批——0.0.6 清单 §⑦ 重整（跨平台当时以侦察项入列，该拍板已被本日 ADR-0011 修订）+ 双证据红绿纪律落地协议/技能文本层（SKILL Evidence 红绿半+一行豁免、comparator 增查两半在场、qa-executor 契约同步、guide 双语 bullet，CLI 零改动）+ 对抗类清单（docs/research-adversarial-checklist.md：九类三态判决 8 已有防护+1 当轮小修、记账新债 3 条带升格条件、OMZ 分类学事实引用+SUL-1.0 边界头注）；计划评审三轮收敛 PASS。跨平台——ADR-0011 拍板 **0.0.6 直接支持三平台分发**（修订「侦察不承诺支持」：三 VM[Parallels Win11 ARM64+Ubuntu ARM] 就绪消解无证据前提；验收线=三 VM 实证+CI matrix 全硬[Windows 测试雷治理前置]；arm64 实证边界、x64 文档声明）+ 四面侦察底稿 docs/research-crossplatform.md（引擎钩子 spawn 语义=strict schema 无平台字段+shell 三态解析[win32 未设→ComSpec 现清单必断链]+process 直启原语；官方下载面三平台×双架构 3.11.2 全在；VM 实探=双平台 node22+试装 doctor 全通+引擎布局同构 `/opt/ZCode/resources/glm/zcode.cjs`+PATHEXT 对偶探针通过；本仓断链恰三处=候选表/启动器/CI 腿）+ 全安装链设计稿 docs/design-crossplatform.md（启动器 C′ 对偶孪生推荐/候选表纯数据扩展/doctor platform 行平台感知，实现棒 crossplatform-support 另立 goal）；AGENTS 三行合一腾位+§8 增「启动器」词条。
- **0.0.6 三平台支持实现棒已落地（2026-09-15，goal crossplatform-support，ADR-0011 验收线收官）**：启动器 C′ 对偶孪生——无扩展名 `run-hook`（POSIX sh）+ `run-hook.cmd`（Windows 孪生），hooks.json 五处一行清单跨双平台（正斜杠引擎形状经 PATHEXT 解析实证）、`run-hook.sh` 退役 + engineCandidates 三平台数据表（win32=`%LOCALAPPDATA%\Programs\ZCode` 装后实测、linux=`/opt/ZCode`，三平台引擎 12,615,227B 同物）+ doctor 平台感知（hook-node 按平台探[start 器/win32 显式过 cmd 防 EINVAL]、platform 行改报候选命中态）+ Windows 测试雷治理（VM 红半 22 挂→绿半 0 挂：ESM 动态 import 绝对路径须 pathToFileURL、win32 homedir 读 USERPROFILE 故隔离面 HOME+USERPROFILE 双 env、启动器契约平台分支、tier2 分隔符断言；.gitattributes 钉行尾）+ CI matrix 增 windows-latest 腿 + 三 VM 活体（Win11 ARM64/Ubuntu aarch64：doctor 双 ✔、PATHEXT 探针、引擎 plugins list hooks:5、scratch loop 全链 finish 双台；SYSTEM 会话装桌面端须 schtasks InteractiveToken 投用户会话）+ README/guide/developers 双语三平台口径+x64/arm64 边界声明；140/140 绿、锚点 21/21 双语、F4 打包 exec 位实证；0.0.6 已发布收官（goal v006-release-mechanics 9/9：活面清扫 11 位点/三体 bump/CHANGELOG 定版/Release notes+runbook 入 checklist/史压缩未推 22 条为单里程碑 ca7e7ed/tag v0.0.6/GitHub Release/npm publish，registry dist-tag latest=0.0.6 实证、CI 四腿全绿[windows stdin 修复 eedb25c]、隔离 prefix 冒烟过；修复教训=comparator 三类抓漏+win32 探针 stdin 阻塞，见 memory）。
