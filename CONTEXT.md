...
**执行策略（execution policy）**：根据任务与项目条件，确定必需检查、评审职责及放行规则的一组要求；其变化不自行改变需求契约或行动授权。
_Avoid_: 执行计划、风险标签、另一份授权

**策略义务（policy obligation）**：执行策略要求当前任务满足的一项检查或评审要求；因影响消失而取消额外义务，须与已取得结果的复用区分。
_Avoid_: 计划步骤、评审发现、权限
...
**派发事务（dispatch transaction）**：队列取用一项待办时在锁内落盘的事件记录（占用登记→结算核销）；崩溃后先与现行目标核对再动作的恢复判据。
_Avoid_: 派发日志、心跳记录、隐式重试

**计量缺席（metering absence）**：一次受控运行查不到对应的完成用量记录；按约定不算零消耗，停止受积分限额约束的自动派发并显式记录。
_Avoid_: 零消耗、用量缺失忽略、事后估算补账
...
**独立评审（independent review）**：由与实现过程分离的评审会话，对需求与候选进行判断；共享必要事实，首轮不共享其他评审结论。其判断与断言—证据相关性核对是不同职责。
_Avoid_: 实现者自检、评审投票、证据对照的同义词

**评审运行（review run）**：一次针对指定候选、承担明确评审职责的实际评审过程，包含可追溯的输入、原始结果与执行结局。
_Avoid_: 导入的结论文件、人工转述的通过声明、正确性证明

**阻塞发现（blocking finding）**：评审中识别出的、在解决前阻止交付的问题；后续评审未再次提及不代表它已解决。
_Avoid_: 当轮评论、自动消失的待办

**独立复核（independent recheck）**：与修复过程分离的评审者，依据原发现、当前候选及对应证据，确认问题已修复或被证伪的判断。
_Avoid_: 实现者标记完成、测试通过即关闭、重复一次全量评审

**评审适用性（review applicability）**：已有评审结论在当前候选上是否仍成立的判断，依据该评审职责及经验证的依赖范围确定；沿用结论不代表发生了新的评审。
_Avoid_: 重新绑定指纹、重新评审、实现者声称无关
