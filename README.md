...
| Bounded queue & cumulative budget | `lzy queue add/list/show/budget/dispatch/reconcile/cancel` | Multiple authorized items run continuously across interruptions (`.lazyzcode/queue/` + `.lazyzcode/budget/`, reset-surviving): state-machine authorization gate, dispatch-transaction crash recovery (reconcile before action), cumulative budget bound to the contract hash that never refreshes across restarts; metering absence / unsettled occupancy is never counted as zero — point-limited dispatch stops with an explicit record (ADR-0027) |
