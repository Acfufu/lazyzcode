...
| Drive workers | `lzy loop drive --workers N` · `--fast` | Multi-worker waves (0.2.4): N worker chains on sibling worktrees, claim-based step split, wave-terminal merge + evidence re-anchor; `--fast` ≡ `--workers 2`; env-auth required, H3R wake switches off; measured turns ≈2× — see the experiment report before enabling |
...
| Verification receipts & scope tier | `lzy verify run/reuse/qualify/list/show/ci` | Controlled executor records checksum receipts (raw output separated) under `.lazyzcode/verify/`; declared-input reuse only after an adversarial qualification, with named conservative fallbacks (ADR-0025); read-only CI check-runs binding with honest not-current/blocked faces |
| Bounded queue & cumulative budget | `lzy queue add/list/show/budget/dispatch/reconcile/cancel` | Multiple authorized items run continuously across interruptions (`.lazyzcode/queue/` + `.lazyzcode/budget/`, reset-surviving): state-machine authorization gate, dispatch-transaction crash recovery (reconcile before action), cumulative budget bound to the contract hash that never refreshes across restarts; metering absence / unsettled occupancy is never counted as zero — point-limited dispatch stops with an explicit record (ADR-0027); since 0.3.1 items may target endpoint B/C with delivery contracts attached and HEAVY items enter the queue on a pre-enqueue plan-review PASS (ADR-0030) |
| Limited delivery B/C | `lzy delivery request/status/act/readback` | Merge-to-main (B) and Pages live verification (C) via per-endpoint delivery contracts riding the UPS approval channel (ADR-0028): merge gated on dual B∧C authorization + PR head/base drift re-check + green CI, bound to the PR head, reads back the actual merge SHA and polls CI on it; C aligns the Pages build commit with the merge SHA and checks live HTTPS content against an expected marker; an intent ledger records identity before any external call and read-back classifies outcomes — no blind re-execution. Since 0.3.1 the queue orchestrates the chain (queue item → merge/verify after the goal finishes; completed only on a done read-back) and action parameters may come from the delivery contract (repo/base/branch/pr-title/pr-body, expect-marker, content-url, multi-page `page:` checks); approvals can be obtained at enqueue time (ADR-0030) |
| Complete migration machine | `lzy migrate preview/apply/status` | Explicit legacy-tree migration (ADR-0029): read-only preview → six-family backup (sha256 manifest) → in-flight draft staging → validation → atomic switch, stamping the `.lazyzcode/state.json` version entry (written last = commit point); in-flight goals become drafts carrying authorization NONE with no scope (not directly registrable — old approvals never escalate); crash-safe and idempotent per task identity via a per-run journal; corrupt goal.json stops before any write, corrupt preserve-family records are warned and kept byte-exact; a live lease/process refuses migration; update/sync never migrate automatically |
