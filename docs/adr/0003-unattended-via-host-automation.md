...
实测发现纯限流反推的窗口会落进平台高峰计价时段（GLM 周一至五 14–18 UTC+8；
DeepSeek 周一至五 9–12 与 14–18——「按建议挂载」恰是最贵运行）。`scheduleAdvisory`
升级为「限流错峰 ∩ 计价感知」：高峰表写死本地（`PEAK_WINDOWS`/`SAFE_WINDOW`，
UTC+8、人工维护、注明官方源与活动期属性），候选窗逐小时对照输出重叠段与
计价安全窗（GLM 夜间活动 23–09：Flash 不限量——活动期条款，过期由文案免责声明）。
