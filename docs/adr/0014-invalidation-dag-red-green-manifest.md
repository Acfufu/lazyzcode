...
**账本校验和与恢复边界（ADJ-43，0.0.10 文档化）**：`dag.json` 的
`checksum` = sha256(canonical JSON)，canonical 载荷=键序固定的
`{"dagVersion":1,"nodes":[…],"edges":[…]}`（`JSON.stringify` 键序=写入序；
loadDag 归一时按同三键重算，文件头额外无关键被忽略）。校验和覆盖载荷本体，
**节点字段顺序在节点对象内不归一**——手改任何载荷字节即校验和不符：该性质=
**抗意外半写**，**不抗能运行同一算法的写者**（重算 checksum 照样写入，防伪边界
见 AGENTS §4 #27）。账本不存在「手工修复」路径：恢复=按 DagError 指路先备份
人工抢救红/waive 记录
（红半边丢失不可重算），然后删除文件重建、绿半由 rebind 重录。形状校验
（0.0.10 ADJ-05）：nodes/edges 须为数组、节点须有 `n<数字>` 形 id 与 kind、
边须有合法 type/from/to——校验和合法而结构畸形同样 fail-closed。
