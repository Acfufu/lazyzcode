...
2. **四阶段+journal 相位记账**：backup（六记录族整树复制+manifest 逐文件 sha256 双读校验）→stage（在途→drafts 草案）→validate（manifest 重读核对）→switch；每相位追加 `migration/journal/<runId>.jsonl`。崩溃后重跑探测未收尾 run 并按任务身份（slug+planHash）幂等续跑——幂等 no-op 路径零新增备份、草案已写不覆盖、state 补齐即收束（恢复跑则产新 runId 全量重备份，旧未收尾 run 的 journal 永久留存为诊断面）。
3. **执法分界（与 A2 对照声明同构）**：goal.json 损坏/未知版本=任何写入前停止；preserve 族（snapshots/salvage/approvals/attestations/evidence/plans）损坏=⚠记录并原样字节保留不阻断（json 族检不可解析、md 族检不可读；内容截断不可机检，如实声明非全量语义检查）——与 M1 预览「执法面在闸不在读」同构：转换面 fail-closed，保留面本就承诺原样。
