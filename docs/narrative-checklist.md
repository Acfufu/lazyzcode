...
- 2026-09-23 v024 刷新（0.2.4 窗口三弧：debt-bundle 搭车 ∥ fast-exp ∥ fast-scheduler；五轮双审 39 条后由 goal v024-fix-round 收口）：测试基线 446→**491**（`npm test` 终树实跑；新增契约含收束族七例/回收矩阵/解析四对照/认领与相位/装配失败/--fast=false）；**计数位点**：README 表 24→**25**/语言（Drive workers 行；help 侧 drive 行同步补 `--workers/--fast`）、guide CLI 栅栏 57/语言不变（workers 旗标入既有 drive 行，不加行）、帮助枚举 loop 族 **23** 不变（同上）、枚举串 22 不变、对比表 11 不变；钩子数 6 不变；doctor 面=workers 残留三桶（可回收/形符无哨兵/归档）+ attest-trailer 措辞收紧。**叙事面新增两处必须区分措辞**：①`--workers` 是 LIGHT-only 实验形态（HEAVY 入口拒，ADR-0026），对外文本不得写成通用并行能力；②屏障重锚是**指纹重锚非复验**（证据文本自带「未复跑断言」），文档不得写成「重跑 F 项验证」。
- 2026-09-23 定版刷新（0.2.3，goal v023-h3r-enforcement + v023-fix-round 维护者直发）：测试基线 404→**446**（本轮 +42：N1 13 + N2 7 + N3 7 + goal A/B 15，`npm test` 实跑）；**五面计数位点零漂移**（README 表 24 行/语言、guide CLI 栅栏 57/语言、帮助枚举 loop 族 23〔账本 24 系 0.2.2 刷新误计，本次已订正——`git diff v0.2.2..HEAD -- cli/` 为空实证本版未动 `cli/`〕、枚举串 22、对比表 11）；钩子数 5→6（PreToolUse 命令层门入列，见上行钩子数行与 2026-09-23 反转记录）；doctor 新增面=无（h3r-words 行 goal A 已入）；发布弧顺延项=ADJ-23 win32 VM 一发（run-hook.cmd 护栏活体，见 release-checklist 0.2.3 节）
- 2026-09-23 **反转记录**（goal v023-h3r-enforcement 落地、v023-fix-round#N4 补账）：上一行「用户文档刻意零 H3R 词条/零 `LZY_ABLATE_*` 开关记载」政策自本日起**不再成立**——H3R 执法点迁移后 guide 双语钩子表如实记载 `h3r-pretool.js`（含 `LZY_ABLATE_H3R_PRETOOL` 开关与其休眠语义），README/guide 的 doctor 行表亦载 `h3r-words`。反转理由：执法点迁移把该原型从纯研究件变为 0.2.3 待发载荷的一部分，诚实披露开关语义（含 ADJ-15 订正的按读者分岔失败语义）优先于「不广告」；形态仍为**休眠原型**，SKILL 句维持 L0 措辞（ADR-0022）。
- 2026-09-26 定版刷新（0.3.0，agent-first 弧 M0–M5 六里程碑；发布机械件维护者指令直发未走 loop）：测试基线 491→**618**（定版树实跑）；**计数位点**：README 表 25→**32**/语言（+7 行：Requirement contracts / Project manifest / Verification receipts / Bounded queue / Limited delivery / Complete migration machine / Migration preview——后两者是**两条独立行**，勿并为一条「migrate」）、guide CLI 栅栏 57→**58**/语言（双语互等 58/58）、帮助枚举 loop 族 **23** 不变（新命令全在顶层，loop 族零新增——`grep -cE '^  lzy loop'` 于 v0.2.4 树与 HEAD 同为 23）、枚举串 **22** 不变（throw 串逐字同形）、对比表 **11** 不变；skills 2 不变但 zw 拆 **8 份 recipes**（knowledge routing，M1）；钩子 6、代理 3 不变；doctor 新增三行 **contract / project / migrate**（push() 名单 diff 实证），另 **approvals 行补入双语 doctor 段**（0.1.x 起该段缺席的旧账，本批顺带清账）。**叙事面新增四条措辞红线**：①update/sync **绝不自动迁移**（ADR-0029）；②迁移草案授权恒 NONE——「旧批准绝不升格」句式不得弱化；③delivery B/C 合并=双授权+漂移复核+CI 门三前置（ADR-0028），不得写成无条件自动合并；④积分预算=近似限制语义（#32）——计量缺席/未计价/在途被杀三类「不算零」显式记录，不得写成精确计量。
