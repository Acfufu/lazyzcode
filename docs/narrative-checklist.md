...
- 2026-09-23 v024 刷新（0.2.4 窗口三弧：debt-bundle 搭车 ∥ fast-exp ∥ fast-scheduler；五轮双审 39 条后由 goal v024-fix-round 收口）：测试基线 446→**本轮实跑数**（见 F5 绿半；新增契约含收束族五例/回收矩阵/解析四对照/认领与相位）；**计数位点**：README 表 24→**25**/语言（Drive workers 行；help 侧 drive 行同步补 `--workers/--fast`）、guide CLI 栅栏 57/语言不变（workers 旗标入既有 drive 行，不加行）、帮助枚举 loop 族 **23** 不变（同上）、枚举串 22 不变、对比表 11 不变；钩子数 6 不变；doctor 面=workers 残留三桶（可回收/形符无哨兵/归档）+ attest-trailer 措辞收紧。**叙事面新增两处必须区分措辞**：①`--workers` 是 LIGHT-only 实验形态（HEAVY 入口拒，ADR-0026），对外文本不得写成通用并行能力；②屏障重锚是**指纹重锚非复验**（证据文本自带「未复跑断言」），文档不得写成「重跑 F 项验证」。
