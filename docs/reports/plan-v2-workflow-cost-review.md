...
### 拍板补录（2026-09-13，开工 session 逐题确认）

| # | 议题 | 结论 |
|---|------|------|
| Q6 | 水位阈值语义 | **常数 + env 覆盖**：初值按本机实测（近 14 天滚动 5h 积分 p95≈1576）上取整写死 1600，`LZY_WATERLINE_POINTS` 覆盖；部件更名「水位警戒线」（「水位 tripwire」与既有 tripwire.js 空转绊线撞名） |
| Q7 | cost 系数口径 | **常设系数 + 促销 overlay**：官方常设规则为底，活动条款走带日期区间 overlay（夜间活动细节本就在 ratelimit.js 维护注释：Flash 不限量/其他翻倍 23–09，至 9/20），退役自然回落常设；实扣为主输出 |
| Q8 | veto 基线起点 | **落地即起算**：埋点上线后第 1 个目标入基线，观测 2 个后 veto 武装；与复挂解耦（09-13 全清后原「切换后起算」语义失去锚点）；db 存量回填积分/步仅作对照 |
| Q9 | 版本边界 | **0.0.3 不发布，直接迭代 0.0.4**：0.0.3 节并入 0.0.4（unreleased），Phase 2 全量计入；npm publish/tag 仍归业主 |
