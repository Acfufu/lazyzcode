...
- **债 H（命令层门的可绕过面，2026-09-23 随本目标记账）**：判定面是命令文本子串 + 记账通道排除 ⇒ 两条理论绕过轴（改写措辞 / 藏进被排除通道）。**升格条件**：狗粮中出现一例「命令层门在线但高危动作仍落地且命令文本明显命中词表」的实例（排除真实规避技巧）——届时要么扩判定面（命令解析/结构匹配），要么把「词表只做提示、不做拦截」的定位收回。现状：不承诺、不扩面（本目标边界=原型 + 实验）。
