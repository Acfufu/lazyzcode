...
| trial | 墙钟 s | 工人墙钟 s | turns（甲+乙） | supersedes（工人窗） | merge 屏障 rebind | lock max ms |
|---|---|---|---|---|---|---|
| f1 serial r1/r2/r3 | 140.8 / 86.7 / 92.2 | 甲 139 / 86 / 91 | 23 / 21 / 21 | 0 / 0 / 0 | 0 / 0 / 0 | 0 / 0 / 0 |
| f1 dual r1/r2/r3 | 146.0 / 201.2 / 138.2 | 甲 144+乙 103 · 甲 199+乙 77 · 甲 92+乙 136 | 31+23 / 39+19 / 20+26 | 1 / 1 / 1 | 1 / 1 / 1 | 0 / 0 / 0 |
| h3 serial r1/r2/r3 | 96.8 / 112.7 / 113.5 | 甲 96 / 112 / 113 | 23 / 23 / 29 | 0 / 0 / 0 | 0 / 0 / 0 | 0 / 0 / 0 |
| h3 dual r1/r2/r3 | 114.1 / 84.8 / 100.5 | 甲 112+乙 112 · 甲 83+乙 79 · 甲 99+乙 65 | 23+16 / 21+17 / 21+15 | 1 / 1 / 1 | 1 / 1 / 1 | 295 / 0 / 0 |

（逐发全字段见 `artifacts/ablation/fast-ledger.jsonl`；各 trial 目录含 trial-meta-fast.json、双工人 stdout、verdict/finish 原文、merge log。**本表 2026-09-23 按账本逐发重打**——旧版把 pilot 发〔`fastpilot-dual-f1-dual-wave-r9`，turns 38+35、工人墙钟 162+181s〕的行混入正式批明细，并有一处跨任务 turns 张冠李戴；勘误见 §7。）
...
- **成本轴（预注册判据外的诚实观测）**：双工臂 turns 中位数 f1 = 54（31+23/39+19/20+26）vs 串行 21 → ≈2.6×；h3 = 38（23+16/21+17/21+15）vs 串行 23 → ≈1.7×（**2026-09-23 按账本重算**；旧版写「h3：46 vs 22；f1：73 vs 21-23」，其中 73=38+35 与 46 均取自/混入 pilot 发）——即使墙钟小赚（h3 11%），按模型请求数计价也是**净亏**，两任务同向。这与 §⑭ 预注册收益模型「墙钟≈最宽独立波」一致，并补上该模型没写的请求成本项。
...
## §7 勘误（2026-09-23，goal v024-fix-round#N11，ADJ-15）

v024 五轮双审查明 §1.2/§1.3 的**明细引用列**有两处失真（判据落判、§1.1 中位数、headline 复算均不受影响——本勘误不改结论，只改引用数字）：

1. **pilot 行混入正式批明细**：`fastpilot-dual-f1-dual-wave-r9`（batch=fastpilot，pilot 发，`taskId` 未定义 bug 修复前的那一发）的 `turns 38+35` 与 `工人墙钟 162+181s` 被写进 §1.2 的「f1 dual」行与 §1.3 的 turns 句（「73 vs 21-23」=38+35 与「h3：46」）。正式批 12 发账本里没有这些数字。
2. **跨任务 turns 张冠李戴**：§1.2「h3 dual」行原写「20+26 等」，账本 h3 dual 三发实为 23+16 / 21+17 / 21+15。
3. **§1.2 表的 supersedes / lock 列粒度**：原表把单值写成「0」或「295 / 0 / 0」，与逐发对齐后按发列出（值本身未变）。

重算后仍成立的**结论**：判据①落判（0.63× / 1.12×）不变；headline「turns≈2×」在重算后成立（f1 ≈2.6×、h3 ≈1.7×，中位跨任务 ≈2×）；锁窗判据③（K=8 max 276ms）与本表无关。账本为唯一事实源，明细列一律自 `artifacts/ablation/fast-ledger.jsonl` 重打。
