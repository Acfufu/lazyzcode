...
// 与 `core/h3r.js` 同读一份）。命中 ⇒ 写 `loop/h3r-hit.json`（带段标；drive 按段校验后才
// 消费，他段/伪造标记不改收束因）+ 返 **deny**——不是 ask：无人值守没有批准者，ask 会挂住
// （`core/drive.js` 同族理由）。
//
...
// 既污染读数又卡住段内收尾。边界如实记账：把高危命令藏进被排除通道是理论绕过轴。
function isBookkeeping(command) {
  const c = command.trim();
  if (/^git\s+(commit|add|status)\b/.test(c)) return true;
  if (/^lzy\b/.test(c)) return true;
  if (/^node\s+\S*cli[\\/]lzy\.js\b/.test(c)) return true;
  if (/^node\s+\S*lzy\.js\b/.test(c)) return true;
  return false;
}
...
try {
  ...
  if (!segmentId) failOpen(); // 无段标=非 drive 派生 ⇒ 免门（车道边界）
